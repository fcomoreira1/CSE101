...
class GameGrid:
    ...
    def __init__(self, nrows, ncols):
        ...
        self.nmines = 0
        self.nflags = 0
        self.tab = [[Square() for i in range(self.ncols)]
    ...
    def neighbors(self, i, j):
       ...
       for x in range(-1 + i, 2 + i):
           for y in range(-1 + j, 2 + j):
               if x == i and y == j:
                   continue
               if 0 <= x and x <= self.nrows -1:
                   if 0 <= y and y <= self.ncols -1:
                       list.append((x,y))
       return list
    ...
    def flags_nearby(self, i, j):
        ...
        for x,y in neighbors:
           flags += self.square_at(x,y).flagged 
        return flags
    ...
    def place_mine(self, i, j):
        ...
        """
        if not self.square_at(i,j).mined:
    ...
    def reveal(self, i, j):
        ...
        if not square.revealed and not square.flagged:
            square.revealed = True
            self.squares_revealed += 1
            if not square.mined and square.mines_nearby == 0:
                for neighbor in self.neighbors(i, j):
                    self.reveal(neighbor[0], neighbor[1])
    ...
    def chording(self, i, j):
        ...
        """
        if not self.mines_nearby(i,j) != self.flags_nearby(i,j):
            for x,y in self.neighbors(i,j):
                if not self.square_at(x,y).flagged:
                    self.reveal(x,y)
    ...
    def random_game(self, n_mines):
        ...
        while self.nmines < n_mines: 
            i = random.randint(0,self.nrows - 1)
            j = random.randint(0,self.ncols - 1)
            self.place_mine(i,j)
...
class MinesweeperApp:
    ...
    def __init__(self, game: GameGrid):
        self.game = game
        self.started = False
    ...
    def left_click_handler(self, i, j):
        ...
        if self.game.squares_revealed == self.game.ncols * self.game.nrows - self.game.nmines:
            self.game_over(True)
    ...
    def right_click_handler(self, i, j):
        ...
        if self.game.revealed(i, j):
            return
        elif not self.game.flagged(i, j):
